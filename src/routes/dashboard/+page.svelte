<script>
	...
	function popup(show, what, mx, my){
		...
		if (what === "quests") {
			quests_popup = show;
			if (thumb_pos < 8){
				y = my*(.15-(thumb_pos/50));
			} else {
				y= my*(.95-(thumb_pos/70));
			}
		} else if (what === "pp") {
			pp_popup = show;
			if (thumb_pos < 8){
...
<div class="content">
	...
	<div class="dashboard" bind:this={dashboard}>
		...
		<div class="line">
			<div class="stats">
				{#if quests_popup}
					<div class="popuptext" style="position:absolute; left:{x*.65}px; top: {y}px;">
						<span>Quests are tasks given to users to help improve pinscout's services in exchange for PinPoints. Weekly quests have a cooldown after completion and refresh every week, while lifetime quests have no cooldown and can be completed infinitely.</span>
					</div>	
				{/if}
				<h2>Available Quests<div class="popup" on:mouseenter={(event)=>popup(true, "quests", event.srcElement.getBoundingClientRect().left, event.srcElement.getBoundingClientRect().top)} on:mouseleave={()=>popup(false, "quests")}>
					<Question />
					</div>:
					<div class="right">4</div></h2>
				<div class="scrollbar">
					<div class="substats">
						...
						<h3 class="taut">↪ PinPoint Reward: <div class="right">+10</div></h3>
						...
						<h3 class="taut">Invite a friend with an invite code. <Question /> <div class="right"></div></h3>
						<h3 class="taut">↪ PinPoint Reward: <div class="right">+50</div></h3>
			...
			<div class="stats">
				{#if pp_popup}
					<div class="popuptext" style="position:absolute; left:{x*.85}px; top: {y}px;">
						<span>PinPoints are a form of digital currency that is given as a reward to users who contribute to the pinscout platform. PinPoints can be spent in the shop.</span>
					</div>	
				{/if}
				<h2>Total PinPoints<div class="popup" on:mouseenter={(event)=>popup(true, "pp", event.srcElement.getBoundingClientRect().left, event.srcElement.getBoundingClientRect().top)} on:mouseleave={()=>popup(false, "pp")}>
					<Question />
					</div>: <div class="right">{userState.user?.points}</div></h2>
				<div class="scrollbar">
					<div class="substats">
						...
						<h3 class="taut">↪ From Friends Invited: <div class="right">+{userState.user?.points}</div></h3>
					</div>
				</div>
			</div>
			<div class="stats">
				<h2>Total Activity: <div class="right">{userState.user?.points}</div></h2>
				<div class="scrollbar">
					<div class="substats">
